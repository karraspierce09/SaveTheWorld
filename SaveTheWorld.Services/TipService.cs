    ...
    public class TipService
    {
        ...
        public bool CreateTip(TipCreate model)
        {
            var entity =
                ...
                {
                    //OwnerID = _userID,
                    // The id is not needed to create a new tip: TipID = model.TipID,
        ...
        public TipDetail GetTipByID(int id)
        {
            using (var ctx = new ApplicationDbContext())
            {
                var entity =
                    ctx
                    .Tips
                    .Single(e => e.TipID == id);      // .Single(e => e.TipID == id && e.OwnerID == _userID);
                return
                    new TipDetail
                    {
                        TipID = entity.TipID,
        ...
        public bool UpdateTip(TipEdit model, int id)
        {
            using (var ctx = new ApplicationDbContext())
            {
                var entity =
                    ...
                    .Single(e => e.TipID == id);      // .Single(e => e.TipID == id && e.OwnerID == _userID); //  .Single(e => e.TipID == model.TipID && e.OwnerID == _userID);
        ...
        public bool DeleteTip(int tipID)
        {
            using (var ctx = new ApplicationDbContext())
            {
                var entity =
                    ctx
                    .Tips
                    .Single(e => e.TipID == tipID);     //.Single(e => e.TipID == tipID && e.OwnerID == _userID);
